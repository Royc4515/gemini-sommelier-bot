...
- **Natural-language orchestrator**: Just say what you want in plain Hebrew ("פתחתי את הפלם", "תוסיף יין", "תמחק את הבקבוק"). The bot classifies the intent and offers a one-tap button to start the right flow; anything that looks like a question is answered as usual. It never writes on its own - every flow keeps its confirmation step. Guessed wrong? One tap ("רק שאלה") answers your original message as chat.
