...
- **Natural-language orchestrator**: Just say what you want in plain Hebrew ("פתחתי את הפלם", "תמחק את צורה", "תוסיף Flam Classico 2021"). The bot parses the intent AND which bottle you meant (matching across languages, so "הפלם" finds a "Flam" row), then acts: a single confirm tap marks the status or deletes; add/edit jump straight into their flow pre-filled. Questions are answered as usual, and it never writes until you confirm. Guessed wrong? One tap ("רק שאלה") answers your original message as chat.
